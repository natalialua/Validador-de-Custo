...
import win10toast
import shutil
from backend.Armazenagem import CalcArmz
...
def update_tx_armz(event):
    ...
    tx_armz_label.config(text="Tx Armazenagem:\n-")

    # if selected_transp:
    #     # Define a tabela com base no tipo selecionado
    #     table_name = "tabelas_lotacao" if selected_type == "Z1" else "tabelas_fracionado"
        
    #     # Buscar o valor da coluna "TX Armazenagem"
    #     tx_armz_result = get_data_from_db(f"""
    #         SELECT "TX ARMAZENAGEM" 
    #         FROM {table_name} 
    ...
    if selected_transp:
        # Define a tabela com base no tipo selecionado
        table_name = "tabelas_lotacao" if selected_type in ["Z1", "E1"] else "tabelas_fracionado"
        ...
        if tx_armz_result:
            tx_armz_value = tx_armz_result[0][0] if isinstance(tx_armz_result[0], (list, tuple)) else tx_armz_result[0]
            formatted_value = f"{tx_armz_value:.2f}"

            tx_armz_label.config(text=f"Tx Armazenagem:\n{formatted_value}")

################################ || ########################################
            
def armazenagem(pai):
    global tp_exped_dropdown, transp_dropdown, entrada_caminho, tx_armz_label
    ...
    tx_armz_label = create_label(canvas, "Tx Armazenagem:", 480, 50)

    tp_exped_dropdown.bind("<<ComboboxSelected>>", update_transp_options)
    transp_dropdown.bind("<<ComboboxSelected>>", update_tx_armz)   
    ...
    global Armazenagem_button_image_1
    Armazenagem_button_image_1 = tk.PhotoImage(file=relative_to_assets("button_2.png"))

    tk.Button(
            image=Armazenagem_button_image_1,
            borderwidth=0,
            highlightthickness=0,
...
def Download_button_clicked(root):
    ...
    try:
        ...
        CalcArmz.main(transportadora, tp_exped)
