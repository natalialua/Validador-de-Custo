...
import win10toast
import shutil
from backend.Estadia import calcE
...
def update_tx_veiculo(event):
    ...
    selected_type = tp_exped_dropdown.get()
    ...
    if selected_transp:
        query = f"""
            SELECT "ESTADIA FIORINO", "ESTADIA VAN", "ESTADIA VUC", 
                   "ESTADIA 3/4", "ESTADIA TOCO", "ESTADIA TRK", "ESTADIA CAR"
            FROM tabelas_fracionado 
...
# Criar o layout da tela Estadia
def Estadia(pai):
    global transp_dropdown, tx_veiculo_label, veiculo_label, entrada_caminho, path_picker_img,  tp_exped_dropdown
    ...
    path_image = path_image.resize((130, 50), Image.LANCZOS)  # Ajuste de tamanho para menor
    ...
    path_label.place(relx=0.15, rely=0.80, anchor="center")
    ...
    inserir_anexo_label.place(relx=0.15, rely=0.80, anchor="center")  
    ...
    path_picker_button2.place(relx=0.22, rely=0.80, anchor="center")
    ...
    path_label1.place(relx=0.40, rely=0.80, anchor="center") 

    referencias_imagens["path_image"] = path_image_tk

    # Criar um rótulo para "Inserir Anexo" um pouco mais abaixo
    inserir_anexo_label1 = tk.Label(canvas, text="Anexo ZSD", bg="#DADADA", fg="#222222", font=("Dm Sans", 7))
    inserir_anexo_label1.place(relx=0.40, rely=0.80, anchor="center") 
    ...
    path_picker_button.place(relx=0.47, rely=0.80, anchor="center")  
   ...
   ############################### Anexo SAP 2 ########################################
    ...
    path_label2 = tk.Label(canvas, image=path_image_tk, bg="#FFFFFF", bd=0)
    path_label2.place(relx=0.65, rely=0.80, anchor="center")  

    referencias_imagens["path_image_ztr"] = path_image_tk

    inserir_anexo_label2 = tk.Label(canvas, text="Anexo ZTR", bg="#DADADA", fg="#222222", font=("Dm Sans", 7))
    inserir_anexo_label2.place(relx=0.65, rely=0.80, anchor="center") 

    path_picker_img_ztr = PhotoImage(file=ASSETS_PATH / "file3.png")

    path_picker_button_ztr = Button(
        canvas,
        image=path_picker_img_ztr,
        borderwidth=0,
        highlightthickness=0,
        command=lambda: selecionar_caminho("sap2", canvas),
        relief='flat',
        bg="#DADADA",
        activebackground="#DADADA"
    )

    path_picker_button_ztr.place(relx=0.72, rely=0.80, anchor="center") 
    path_picker_button_ztr.lift()

    referencias_imagens["path_picker_img_ztr"] = path_picker_img_ztr

   ################### Gerar ###################################
    global Estadia_button_image_1
    Estadia_button_image_1 = tk.PhotoImage(file=relative_to_assets("button_2.png"))

    tk.Button(
            image=Estadia_button_image_1,
            borderwidth=0,
            highlightthickness=0,
...
def selecionar_caminho(tipo_anexo, canvas):
    global caminho_saida_email, caminho_saida_sap, caminho_saida_sap2, label_email, label_sap, label_sap2
    ...
    elif tipo_anexo == "sap2":
        filetypes = [("Text files", "*.txt")]
        title = "Selecione um arquivo TXT para anexar ao SAP 2"
    else:
    ...
    if caminho_selecionado:
            ...
            if tipo_anexo == "email":
                ...
                if label_email:
                    ...
                    label_email.place(relx=0.16, rely=0.90, anchor="center")
                ...
                if label_sap:
                    ...
                    label_sap.place(relx=0.4, rely=0.90, anchor="center")
            
            elif tipo_anexo == "sap2":
                caminho_saida_sap2 = caminho_selecionado
                nome_arquivo_sap2 = os.path.basename(caminho_saida_sap2)
                
                if label_sap2:
                    label_sap2.config(text=nome_arquivo_sap2)
                else:
                    label_sap2 = tk.Label(canvas, text=nome_arquivo_sap2, bg="#FFFFFF", fg="#222222", font=("Dm Sans", 7))
                    label_sap2.place(relx=0.65, rely=0.90, anchor="center")
...
def salvar_anexos():
    ...
    if caminho_saida_sap2:
        destino_sap2 = Path(PASTA_DESTINO) / Path(caminho_saida_sap2).name
        shutil.copy(caminho_saida_sap2, destino_sap2)
        print(f"Arquivo do SAP 2 salvo em: {destino_sap2}")

def Download_button_clicked(root):
    """ Função chamada ao clicar no botão de download. """
    global caminho_saida_email, caminho_saida_sap, caminho_saida_sap2, tp_exped_dropdown, transp_dropdown

    if not caminho_saida_email and not caminho_saida_sap and not caminho_saida_sap2:
    ...
    try:
        ...
        if caminho_saida_sap2:
            file_name_sap2 = os.path.basename(caminho_saida_sap2)
            shutil.copy(caminho_saida_sap2, os.path.join(caminho_destino, file_name_sap2))
            print(f"Arquivo do SAP 2 salvo em: {caminho_destino}/{file_name_sap2}")
        ...
        calcE.main(tp_exped, transportadora, file_name_email, file_name_sap, file_name_sap2)
